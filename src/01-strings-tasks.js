...
function repeatString(value, count) {
  return value.padStart(value.length * count, value);
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (s) => s2[s1.indexOf(s)]);
}
...
function getCardId(value) {
  const offset = '♣♦♥♠'.indexOf(value.slice(-1));
  ...
  return parseInt(value, 10) - 1 + 13 * offset;
}
